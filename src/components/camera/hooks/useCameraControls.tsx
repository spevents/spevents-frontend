...
export const useCameraControls = ({ stream }: CameraControlsOptions) => {
  ...
  const setFocusPoint = useCallback(async (x: number, y: number) => {
    if (!stream) return;
    ...
    try {
      ...
      if ('focusMode' in capabilities && Array.isArray(capabilities.focusMode) && capabilities.focusMode.includes('manual')) {
  ...
  const setExposure = useCallback(async (value: number) => {
    ...
    try {
      ...
      if ('exposureMode' in capabilities && Array.isArray(capabilities.exposureMode) && capabilities.exposureMode.includes('manual')) {

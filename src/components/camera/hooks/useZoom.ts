...
export const useZoom = (facingMode: "user" | "environment" = "environment") => {
  ...
  const [deviceZoomCapabilities, setDeviceZoomCapabilities] =
    useState<ZoomCapabilities | null>(null);
  const initialDeviceZoomRef = useRef<number | null>(null);

  const getDeviceZoomCapabilities = async (
    track: MediaStreamTrack,
  ): Promise<ZoomCapabilities | null> => {
    try {
  ...
  const applyZoom = useCallback(
    (track: MediaStreamTrack, zoom: number) => {
      ...
      if (zoom === DEFAULT_ZOOM) {
        track
          .applyConstraints({
            advanced: [
              {
                zoom: initialDeviceZoomRef.current,
      ...
      if (deviceZoom !== undefined) {
        track
          .applyConstraints({
            advanced: [
              { zoom: deviceZoom } as unknown as MediaTrackConstraints,
            ],
          })
          .catch((error) => {

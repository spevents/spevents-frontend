...
export function QRCodeModal({ isOpen, onClose }: QRCodeModalProps) {
  ...
  const handleUpdateNgrokUrl = () => {
    const newUrl = prompt(
      "Enter your ngrok URL (e.g., https://xxxx-xxx-xx-xxx-xx.ngrok.io)",
    );
    if (newUrl) {
      setIsUpdating(true);
      setBaseUrl(newUrl);
  ...
  const handleUpdateSessionCode = () => {
    const newCode = prompt("Enter your custom session code");
    if (newCode) {
  ...
  const getLiveUrl = () => {
    return sessionCode
      ? `https://join.spevents.live/${sessionCode}/guest/camera`
      : "";
  };

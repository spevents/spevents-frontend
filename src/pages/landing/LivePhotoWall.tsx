...
import { Monitor, LayoutTemplate, Presentation, Hotel } from "lucide-react";
...
export const LivePhotoWall = ({
      ...
      <AnimatePresence>
        {displayPhotos.map((photo, index) => (
          <motion.div
            ...
            className="absolute w-20 h-20 rounded-lg overflow-hidden shadow-lg border-2 border-white"
          >
            <img
              src={photo.src}
  ...
  const renderPresenterMode = () => {
    const photoSets = [];
    for (let i = 0; i < displayPhotos.length; i += 3) {
      photoSets.push(displayPhotos.slice(i, i + 3));
    }
          ...
          >
            ...
            {displayPhotos.length > 1 && (
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 space-y-2">
                {displayPhotos.slice(-4, -1).map((photo, index) => (
                  <motion.div
                    key={photo.id}
